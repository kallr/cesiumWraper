...
namespace SIMPLE
{
	std::pair<int, wraperCesium::GMesh>  simple_Function(int id, const wraperCesium::GMesh& mesh, double factor, bool bKeepBorder)
	{
		wraperCesium::GMesh newMesh = mesh;
		newMesh.batchIDs.clear();
		...
		toMesh_func(newMesh, m, mask);

		bool flag = true;

		double error = 0;
		flag = quadric_simplification(error, mm, factor, bKeepBorder);

		if (flag)
			fromMesh_func(newMesh, m);
		return { id,newMesh };
	}
	...
	bool SimpleMeshByFator_func(wraperCesium::GMesh& mesh, double factor, double& targetError)
	{
		...
		if (1)
		{
			...
			std::unordered_map<int, wraperCesium::GMesh> objIDs;
			std::vector<std::future<std::pair<int, wraperCesium::GMesh> > > results;
			...
			{
				int id = item.first;
				wraperCesium::GMesh& tMesh = item.second;
				if (threadCount > 1) {
					results.emplace_back(pool.enqueue([id, tMesh, factor, bKeepBorder] {
						return simple_Function(id, tMesh, factor, bKeepBorder);
					}));
				}
				else
				{
					simple_Function(id, item.second, factor, bKeepBorder);
				}
			}
			...
			for (auto & result : results)
			{
				std::pair<int, wraperCesium::GMesh> oneInfo = result.get();
				//
				objIDs[oneInfo.first] = oneInfo.second;
			}
		}
		else
		{
			for (auto& item : objIDs)
			{
				...
				toMesh_func(item.second, m, mask);

				bool flag = true;

				double error = 0;
				if (abs(mesh.batchIDs[0] - 9999999) < 0.1)
					flag = quadric_simplification(error, mm, factor, true);
				else
					flag = quadric_simplification(error, mm, factor, false);

				if (error > targetError)
					targetError = error;

				if (flag)
					fromMesh_func(item.second, m);
			}
